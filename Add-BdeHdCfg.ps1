...
if (((Get-WindowsEdition -Online).Edition -match '^Server.+Cor$') -and ($bdehdcfgURI.length -gt 0)) {
    ...
    if (-not (Test-Path ('c:\\windows\\system32\\BdeHdCfg.exe')) -or -not (Test-Path ('c:\\windows\\system32\\BdeHdCfgLib.dll'))) {
        $bdehdcfgURI -imatch '.+/(.+)$' | Out-Null
        #$bdehdcfgZIP = $env:TEMP + '\\' + $matches[1]
        $bdehdcfgZIP = 'c:\\temp\\' + $matches[1]
        ...
        try {
            ...
            Expand-Archive -LiteralPath $bdehdcfgZIP -DestinationPath 'c:\\windows' -ErrorAction SilentlyContinue
        }
        catch {
            #Write-Host (get-date -DisplayHint Time) failed to expand $bdehdcfgZIP to $env:windir / most likely the files already exist
            Write-Host (get-date -DisplayHint Time) failed to expand $bdehdcfgZIP to 'c:\\windows' / most likely the files already exist
        }
        # Removing temp files
